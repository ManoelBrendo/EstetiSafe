...
const faqItems = [
  ...
  },
  {
    ...
    answer: 'Ele aparece na organização documental, nos alertas, nos registros auditáveis e na forma como a clínica passa a acompanhar a regularização sanitária em um só fluxo.',
  },
  {
    id: 'implantacao',
    title: 'Esse posicionamento comercial sustenta a entrega depois da venda?',
    answer: 'Sim. A landing vende uma plataforma sofisticada, mas a base do produto continua sendo rotina organizada, atendimento elevado e acompanhamento sanitário real.',
  },
  {
    id: 'whitelabel',
...
function resolveBranding() {
  ...
  const tagline = cleanTextValue(params?.get('brandTagline')) || cleanTextValue(env.VITE_MARKETING_BRAND_TAGLINE) || 'Software para clínicas de estética'
  const clinicLabel = cleanTextValue(params?.get('clinicLabel')) || cleanTextValue(env.VITE_MARKETING_CLINIC_LABEL) || 'Interface sofisticada com acompanhamento sanitário'
  const logo = cleanLogoValue(params?.get('brandLogo')) || cleanLogoValue(env.VITE_MARKETING_BRAND_LOGO) || logoPath
  const poweredBy = cleanTextValue(params?.get('poweredBy')) || cleanTextValue(env.VITE_MARKETING_POWERED_BY) || "Powered by L'Appui"
...
export default function MarketingSite() {
  ...
  const supportContact = useMemo(() => getSupportContact(), [])
    ...
    <div className="leadv2-page">
      <header className="leadv2-topbar">
        <Link to="/" className="leadv2-brand">
          <span className="leadv2-brand-mark"><img src={branding.logo} alt={branding.logoAlt} /></span>
      ...
      <main className="leadv2-main">
        ...
        <section id="modulos" className="leadv2-section leadv2-modules-section">
          <div className="leadv2-section-head">
            ...
            <h2>Uma plataforma longa o suficiente para sustentar a operação e elegante o suficiente para elevar a percepção do atendimento.</h2>
